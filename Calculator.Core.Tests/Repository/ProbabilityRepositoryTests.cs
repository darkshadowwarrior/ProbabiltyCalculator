...
namespace Calculator.Core.Repository
{
    public class ProbabilityRepositoryTests
    {
        ...
        [Theory]
        [InlineData(0.75, "CombinedWith", new double[] { 0.5, 0.5 })]
        public void ShouldSaveToFileWhenSaveIsCalled(double itemToSave, string typeOfCalculation, double[] Inputs)
        {
            ...
            var expectedLineToSave = $"Date calculation processed is: {probabilityToSave.Date} , Type of calculation: {probabilityToSave.TypeOfCalculation}, Result of calculation: {probabilityToSave.Value}, Inputs used are: P1 = {probabilityToSave.Inputs[0]} and P2 = {probabilityToSave.Inputs[0]}";
